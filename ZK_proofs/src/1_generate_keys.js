...
const generator = async (circuit) => {
    console.info('Initializing ZKey process...')
     ...
     await snarkjs.zKey.newZKey(`./circuit/${circuit}.r1cs`, "./keys/powerOfTau_final.ptau", `./keys/${circuit}_00.zkey`)
    ...
    const flag = await snarkjs.zKey.verifyFromR1cs(`./circuit/${circuit}.r1cs`,"./keys/powerOfTau_final.ptau",`./keys/${circuit}_final.zkey`)
    if (!flag){
...
generator('ExactArrayOfTokens').then((circuit) => {
    process.exit(0)
});
